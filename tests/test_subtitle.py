...
from video_translate.subtitle import SubtitleWriter, SubtitleReader
from video_translate.config import SubtitleConfig
from video_translate.models import SubtitleSegment, SubtitleFormat
...
class TestSubtitleWriter:
    ...
    def test_write_multiple_segments(self, temp_dir):
        """测试写入多个字幕片段"""
        segments = [
            SubtitleSegment(index=i, start=i*2.0, end=(i+1)*2.0, 
                          text=f"Text {i}", translated=f"文本 {i}")
            for i in range(1, 6)
        ]
